 ...
 * hook-chapter · 完整章節範例
 * ─────────────────────────────────────────
 * 範例寫於一個報刊型主題下（襯線 + 單一紅 accent + 印刷蓋章 motion）。
 *
 * 關鍵手段：
 * - 真素材：<img src="/hook/{name}.png" /> 而不是 placeholder
 * - 字級狠對比：hero 用 --t-display-1（≥ 144px）+ 微微負字距
 * - 主導動作：mask reveal + 印章砸下（貼合該主題的印刷調性）
 * - takeover：三張圖縮入 + 巨字爆出 + accent 紅條貫穿
 * - 收束：brush 劃掉舊概念
 *
 * 換其它主題時依那個主題的調性自由換「印章砸下 / brush」等效動作，
 * 結構和字級節奏保持。
 */
export default function HookChapter({ step }: ChapterStepProps) {
  ...
  // ⚠️ 這是結構範例。具體反例 caption / src 應該來自 outline.md 本章
  //    article 補欄位（雙來源原則）—— 別照抄下面這些佔位字串。
