...
第一步不渲染返回、最後一步不渲染前進，Auto 錄影模式全部隱藏。控制項造型與
顏色使用 `--nav-*` 主題 token；主題未提供時才採用 `Stage.css` 的通用 fallback。
...
| 5 | 隱藏的邊角控制項 | 進度列預設 opacity 0；底邊 hover 顯示章節列，左右全高度感應帶 hover 顯示 step 導覽箭頭 |
