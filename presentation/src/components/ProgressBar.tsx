...
interface Props {
  ...
  /**
   * Optional GitHub link rendered in the tool bar; reveals/hides together
   * with the chapter list on hover. Pass `null` to hide.
   */
  githubUrl?: string | null;
  /** Start the PDF export. Omit to hide the export button. */
  onExport?: () => void;
 ...
 *
 * A tool bar (source link + PDF export) sits to the right of the viewport,
 * sharing the same hover trigger so it appears/disappears in sync with the bar.
 */
  ...
  return (
    <div className="pb-hover" data-no-advance>
              ...
              >
                ...
                >
                  ...
                  <span
                    className="pb-title"
                    title={c.title}
                    // The pill grows on hover to reveal the full title; if that
                    // pushes its tail past the scrolling track, nudge it in.
                    onTransitionEnd={(e) => {
      ...
      <div className="pb-tools">
        {onExport && (
          <button
            className={`pb-tool pb-export ${exporting ? "is-busy" : ""}`}
            type="button"
            disabled={exporting}
            aria-label={exporting ? "準備列印中" : "匯出成 PDF"}
            title={exporting ? "準備列印中…" : "匯出成 PDF（每段一頁）"}
            onClick={(e) => {
              e.stopPropagation();
              onExport();
            }}
        ...
        {githubUrl && (
          <a
            className="pb-tool pb-github"
            href={githubUrl}
            target="_blank"
            rel="noopener noreferrer"
            aria-label="在 GitHub 查看原始碼"
            title="在 GitHub 查看原始碼"
