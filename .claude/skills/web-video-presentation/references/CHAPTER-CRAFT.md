...
**每支片子的第一个章节固定是封面** —— `src/chapters/00-cover/`，
内容章节从 `01-` 起。它是录屏的第一帧，决定观众对整片气质的第一印象，
所以跟第 1 章一起在主线程做完、一起给用户验收（见
...
**建议有**：重点清单预告（这支片子会讲几件事）、一句总结句。

**版式怎么定**：主题 `theme.json` 若有 `styleReference`，**先看那张参考图**
（脚手架已复制到 `<project>/public/theme-assets/`），照它的 `layoutNote`
定版式再落地。没有参考图就自己按主题气质设计。

**仍然受全片红线约束**：

- 没有页眉 / 页脚 / 页码 / 品牌条
- 字号大、留白多；颜色与字体家族走 token
- **至少一处动效** —— 封面允许以静态构图为主（它要让人看清标题），
  但不能完全没动作。一条横线划出、标题分行错峰进、角色图轻轻落位，
  任选一种即可，不要三种都上
- **`narrations.ts` 照样必须有**。没有开场口播就用空字符串 `""`，
  Auto 模式会按字数估时撑过去

**封面不受"每章 3~8 步"的经验约束** —— 1~2 步就够。
...
- 每屏右下角都挂 mono 角标 / 序号
- 生成插图里带文字 / 假 logo / 拼错的招牌字（文字一律留在 HTML，
  见 [`ILLUSTRATIONS.md`](ILLUSTRATIONS.md)）
...
- [ ] **outline 声明的「插图描述」全部已生成并接入**，或已降级 placeholder
      **且交付时明确告诉了用户**；`outline.md` 素材清单已回写（`🎨` → `✓` / `⚠️`）
- [ ] **生成插图的画风与主题 `styleReference` 及既有 `illustrations` 一致**，
      且图内没有任何文字 —— 不一致就重新生成，不将就
- [ ] **封面章节（若本次做的是 `00-cover`）**：主标题 / 副标题 / 讲者或出处
      齐全，至少一处动效，无页眉页脚
...
- [ ] **`pnpm exec tsc --noEmit`（或 `npx tsc --noEmit`）通过** —— 不通过禁止汇报"做完了"
